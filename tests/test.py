...
from config import FileNotExistsError
from config import SectionMissingError
from config import MandatoryOptionMissingError
from database import EmailAlreadyInUseError, EmailTooLongError
import crypto


class TestConfig:
    #if set to true, the database tables will be truncated on each test run
    TRUNCATE_TABLES = False
    EMAIL_ADDRESS = 'janosch.woschitz@gmail.com'
    #runs a test which sends a mail to the configured email address
    SEND_TEST_MAIL = False
    #runs a test which sends a mail via ttls to the configured email address
    SEND_TTLS_MAIL = False
    TTLS_PORT = 587
...
class ConfigTest(unittest.TestCase):
    ...
    def test_create_config_from_file(self):
        self.failUnlessRaises(
            FileNotExistsError,
            auth.Config,
            Files.NON_EXISTING_CONFIG
        )
        self.failUnlessRaises(
            SectionMissingError,
            auth.Config,
            Files.CONFIG_WITH_MISSING_SECTION
        )
        self.failUnlessRaises(
            MandatoryOptionMissingError,
            auth.Config,
            Files.CONFIG_WITH_MISSING_OPTION
        )
        ...
        for name in parser.options(auth.Config.CONFIG_SECTION_NAME):
            ...
            c_item = filter(
                lambda config_item: config_item.name == name,
                config._config_items
            )[0]
            if c_item.convert_func:
                self.assertEqual(
                    getattr(config, name),
                    c_item.convert_func(value)
                )
            else:
...
class DatabaseTest(unittest.TestCase):
    ...
    def test_configure(self):
        ...
        self.assertEqual(
            context.user.exists('some.random_mail_address@provider.com'),
            False
        )
...
class TestMailDispatcher(auth.MailDispatcher):
    ...
    def send_mail(self, from_addr, receiver_addr, subject,
                  template_fill_args_dictionary):
        token = "{ACTIVATION_TOKEN}"
        self.activation_token = template_fill_args_dictionary[token]
...
class MailTest(unittest.TestCase):
    def test_send(self):
        ...
        template_dispatcher = auth.TemplateMailDispatcher(
            mail_config,
            config.mail_body
        )

        template_dispatcher_with_html = auth.TemplateMailDispatcher(
            mail_config,
            config.mail_body,
            u"<b>Some HTML and \xdcnic\xF6d\xC9 <b><br/>Token: {ACTIVATION_TOKEN}</br><span style=\"color:red\"><b>red text</b></span>"
        )

        if TestConfig.SEND_TEST_MAIL:
            dispatcher.send_mail(
                from_addr,
                TestConfig.EMAIL_ADDRESS,
                "This is a test",
            ...
            dispatcher.send_mail(
                from_addr,
                TestConfig.EMAIL_ADDRESS,
                u"\xdcnic\xF6d\xC9 test",
                u"some chars: \xD0\xCE\xC9\xC6"
            )
            template_dispatcher.send_mail(
                from_addr,
                TestConfig.EMAIL_ADDRESS,
                config.mail_subject,
                {"{ACTIVATION_TOKEN}": "1234567890"}
            )
            template_dispatcher_with_html.send_mail(
                from_addr,
                TestConfig.EMAIL_ADDRESS,
                config.mail_subject,
                {"{ACTIVATION_TOKEN}": "1234567890"}
...
class ValidatorTest(unittest.TestCase):
    def test_validate_email(self):
        self.assertEqual(
            auth.validator.is_valid_email('test_test'),
            False
        )
        self.assertEqual(
            auth.validator.is_valid_email('j\xC9rgen.schmidt@test.de'),
            False
        )
        self.assertEqual(
            auth.validator.is_valid_email('juergen-schmidt@test.de'),
            True
        )
        self.assertEqual(
            auth.validator.is_valid_email('juergen+schmidt@test.de'),
        ...
        self.assertEqual(
            auth.validator.is_valid_email('juergen.schmidt@test.d'),
            False
        )
        self.assertEqual(
            auth.validator.is_valid_email('juergen%schmidt@test.deeeeeee'),
            False
        )
...
class AuthTest(unittest.TestCase):
    ...
    def test_create(self):
        ...
        self.assertEqual(
            auth_handler._db_context.user.exists(TestConfig.EMAIL_ADDRESS),
            True
        )
        self.assertEqual(user.login(password), False)
        self.assertEqual(
            user.login_result,
            auth.LoginResult.USER_IS_NOT_ACTIVATED
        )
    ...
    def test_failed_logins(self):
        ...
        def get_login_count():
            return db_context.login_attempt.get_count(
                user.email,
                config.login_attempt_expire
            )

        self.assertEqual(get_login_count(), 3)
        # inserting outdated login attempt, should not influence count
        self._insert_login_attempt_with_date(
            config,
            user.email,
        ...
        self.assertEqual(
            user.login_result,
            auth.LoginResult.USER_OR_PASSWORD_WRONG
        )
        user.login(wrong_password)
        user.login(wrong_password)
        self.assertEqual(
            user.login_result,
            auth.LoginResult.USER_IS_LOCKED_OUT
        )
        self.assertEqual(get_login_count, 6)
    ...
    def test_auth_settings(self):
        ...
        value = int(parser.get(
            auth.Config.CONFIG_SECTION_NAME,
            'password_min_length')
        )
